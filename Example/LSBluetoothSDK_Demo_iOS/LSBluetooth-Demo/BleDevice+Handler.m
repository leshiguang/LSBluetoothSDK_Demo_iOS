...
+(void)deleteBleDevice:(BleDevice *)delDevice inManagedObjectContext:(NSManagedObjectContext *)context
{
    if(delDevice)
    {
        ...
        {
            NSLog(@"done of delete ble device........");
        }
        else NSLog(@"failed to delete ble device........");
    }
}
            ...
            inManagedObjectContext:(NSManagedObjectContext *)context
{
    if(lsDeviceInfo && context)
    {
        ...
        else if(lsDeviceInfo.deviceId.length)
        {
            predicate=[NSPredicate predicateWithFormat:@"deviceID = %@",lsDeviceInfo.deviceId];
            request.predicate = predicate;
        }
        else if(lsDeviceInfo.broadcastId.length)
        {
            predicate=[NSPredicate predicateWithFormat:@"broadcastID = %@",lsDeviceInfo.broadcastId];
            request.predicate = predicate;
        }
